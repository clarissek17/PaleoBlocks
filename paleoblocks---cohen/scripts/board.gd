extends Node

class_name Board

signal tetromino_locked

const ROW_COUNT = 20
const COLUMN_COUNT = 10

var tetrominos:Array[Tetromino] = []
@export var tetromino_scene: PackedScene

### NEW CODE
@export var dirt_scene: PackedScene # Assign Dirt.tscn in the Inspector
var dirt_blocks = []

func _ready():
	#spawn_dirt_layer()
	pass
	
func start_board():
	spawn_dirt_layer()

func spawn_dirt_layer():
	var cell_size = 16 # piece size
	var start_x = -72 # where spawning starts
	var start_y = 168 - (9 * cell_size) # 12 rows
	
	for row in range(12):
		for col in range(COLUMN_COUNT):
			var dirt = dirt_scene.instantiate()
			add_child(dirt)
			# Position them relative to the board grid
			var x_pos = start_x + (col * cell_size)
			var y_pos = start_y + (row * cell_size)
			
			dirt.position = Vector2(x_pos, y_pos)
			dirt_blocks.append(dirt)
###


func spawn_tetromino(type: Shared.Tetromino, is_next_piece, spawn_position):
	var tetromino_data = Shared.data[type]
	var tetromino = tetromino_scene.instantiate() as Tetromino
	
	tetromino.tetromino_data = tetromino_data
	tetromino.is_next_piece = is_next_piece
	
	tetromino.lock_tetromino.connect(on_tetromino_locked)
	
	if is_next_piece == false:
		tetromino.position = tetromino_data.spawn_position
		tetromino.other_tetrominos = tetrominos
		
		add_child(tetromino)
	else:
		tetromino.queue_free()
var delayer = 0
func on_tetromino_locked(tetromino: Tetromino):
	tetrominos.append(tetromino)
	tetromino_locked.emit()
	#check game over
	
	# Look at every 'Piece' child inside the Tetromino that just locked 
	for piece in tetromino.get_children():
		if piece is Piece: 
			
			if piece.global_position.y < -150: 
				get_parent().get_node("PieceSpawner").trigger_game_over()
				return # Stop the rest of the function so we don't spawn more pieces
	
	#check for clear lines
	clear_lines()
	

func clear_lines():
	var board_pieces = fill_board_pieces()
	clear_board_pieces(board_pieces)
	
func fill_board_pieces():
	var board_pieces = []
	
	for i in ROW_COUNT:
		board_pieces.append([])
		
	for tetromino in tetrominos:
		var tetromino_pieces = tetromino.get_children().filter(func (c): return c is Piece)
		for piece in tetromino_pieces:
			var row =  (piece.global_position.y + piece.get_size().y / 2) / piece.get_size().y + ROW_COUNT / 2
			board_pieces[row - 1].append(piece)
	return board_pieces

func clear_board_pieces(board_pieces):
	var i = ROW_COUNT
	while i > 0:
		var row_to_analyze = board_pieces[i - 1]
		
		if row_to_analyze.size() == COLUMN_COUNT:
			clear_row(row_to_analyze)
			board_pieces[i - 1].clear()
			move_all_board_pieces_down(board_pieces, i)
		i -= 1

func clear_row(row):
	for piece in row:
		piece.queue_free()

func move_all_board_pieces_down(board_pieces, cleared_row_number):
	for i in range(cleared_row_number -1, 1, -1):
		var row_to_move = board_pieces[i - 1]
		if row_to_move.size() == 0:
			return false
		for piece in row_to_move:
			piece.position.y += piece.get_size().y
			board_pieces[cleared_row_number - 1].append(piece)
		board_pieces[i - 1].clear()
